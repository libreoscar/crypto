...
type hash256Test struct {
	out string
	in  string
}

var golden = []hash256Test{
...
func TestGolden(t *testing.T) {
	for i := 0; i < len(golden); i++ {
		g := golden[i]
		s := fmt.Sprintf("%x", Hash256([]byte(g.in)).Data)
		if s != g.out {
			t.Fatalf("Sum256 function: Hash256(%s) = %s want %s", g.in, s, g.out)
		}
		c := NewHasher256()
		for j := 0; j < 3; j++ {
			if j < 2 {
				...
				c.Feed(([]byte)(g.in[0 : len(g.in)/2])).Sum(nil)
				c.Feed(([]byte)(g.in[len(g.in)/2:]))
			}
			s := fmt.Sprintf("%x", c.Sum(nil).Data)
			if s != g.out {
...
func benchmarkSize(b *testing.B, size int) {
	...
	for i := 0; i < b.N; i++ {
		bench.Reset().Feed(buf[:size]).Sum(sum[:0])
	}
}
